...
const work = mkdtempSync(path.join(tmpdir(), 'use-validation-pack-'));
...
try {
  ...
  if (!result?.filename) throw new Error('npm pack --json did not report a filename');

  const packedFiles = result.files.map((file) => file.path);
  const unexpected = packedFiles.filter((file) => !expectedFiles.includes(file));
  ...
  execFileSync('tar', ['-xzf', path.join(work, result.filename), '-C', work], {
    stdio: 'inherit',
  });
  ...
  symlinkSync(path.join(root, 'node_modules/react'), path.join(nodeModules, 'react'), 'dir');
  symlinkSync(
    path.join(root, 'node_modules/react-use-reactive'),
...
if (typeof useValidation !== 'function') {
  throw new Error('package entry does not export useValidation');
}
